...
public class TShortName {
	...
	public TShortName() throws Exception{
		Connection connection = ConnDB.getConnection("Tes");
		PreparedStatement preparedStatement = null;
		preparedStatement = connection.prepareStatement("insert into DataWeb(title,abstract) values(?,?)");
...
//		HashMap<String, Result> reHashMap = new HashMap<String,Result>();
		try {
			String sear = "toothache";
			//这种方法才能实现查询
			htmlPage = webclient.getPage("http://www.ncbi.nlm.nih.gov/pubmed?term="+sear);
...
//						Result result = new Result();
						...
						if(domElement!=null){
							String title = domElement.asText();
							preparedStatement.setString(1, title);
							System.out.println(ii+" "+title);
						}
						if(domNode!=null){
							String abstraString = domNode.asText();
							System.out.println(abstraString);
							preparedStatement.setString(2, abstraString);
						}
						ii++;
